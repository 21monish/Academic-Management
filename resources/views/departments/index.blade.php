<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('departments.create') }}" class="px-4 py-2 bg-indigo-600 text-white rounded-md text-sm">+ Add Department</a>
        </div>
    </x-slot>
    ...
    <div class="py-8 max-w-6xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="bg-white shadow-sm rounded-lg overflow-hidden mb-6 p-4 border border-gray-100">
            <form method="GET" action="{{ route('departments.index') }}" class="grid grid-cols-1 md:grid-cols-6 gap-3">
                <div class="{{ $showCollegeFilter ? 'md:col-span-2' : 'md:col-span-4' }}">
                    <x-input-label for="q" value="Search" />
                    <x-text-input id="q" name="q" class="block mt-1 w-full" :value="request('q')" placeholder="Code, name, description" />
                </div>
                ...
                <div class="flex items-end gap-2">
                    ...
                    @if(request()->hasAny(['q', 'university_id', 'college_id', 'is_active']))
                        <a href="{{ route('departments.index') }}" class="px-4 py-2 bg-gray-100 text-gray-700 rounded-md text-sm">Clear</a>
                    @endif
                </div>
            </form>
        </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">HOD</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse ($departments as $department)
                        <tr>
                            ...
                            <td class="px-4 py-2">{{ $department->college?->name }}</td>
                            <td class="px-4 py-2">{{ $department->hod?->full_name ?? '-' }}</td>
                            <td class="px-4 py-2">{{ $department->is_active ? 'Active' : 'Inactive' }}</td>
                            <td class="px-4 py-2 text-right space-x-2">
                                <a href="{{ route('departments.edit', $department) }}" class="text-indigo-600 text-sm">Edit</a>
                    ...
                    @empty
                        <tr><td colspan="6" class="px-4 py-6 text-center text-gray-500">No departments found.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
